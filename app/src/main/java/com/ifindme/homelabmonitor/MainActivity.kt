    ...
    private inner class MonitorView : View(this@MainActivity) {
        ...
        override fun onDraw(canvas: android.graphics.Canvas) {
            ...
            if (System.currentTimeMillis() - lastShift > 180_000) {
                offsetX = when (offsetX) {
                    0 -> 2
                    2 -> -2
                    else -> 0
                }
                offsetY = when (offsetY) {
                    0 -> 1
                    1 -> -1
                    else -> 0
                }
            ...
            if (s != null && !offline) {
                paint.textSize = 19f
                paint.color = Color.WHITE
                canvas.drawText("CPU    " + percent(s.cpu), x, y, paint)
                y += 30f
                canvas.drawText("RAM    " + percent(s.ram), x, y, paint)
                y += 30f
                canvas.drawText("DISK   " + percent(s.disk), x, y, paint)
                y += 30f
                canvas.drawText("UPTIME " + uptime(s.uptime), x, y, paint)
                y += 46f

                paint.textSize = 13f
                paint.color = Color.rgb(130, 130, 130)
                canvas.drawText("NETWORK", x, y, paint)
                y += 30f

                paint.textSize = 19f
                paint.color = Color.WHITE
                canvas.drawText("RX     " + bytes(s.rxBytes), x, y, paint)
                y += 30f
                canvas.drawText("TX     " + bytes(s.txBytes), x, y, paint)
                y += 48f
                ...
                paint.textSize = 19f
                paint.color = Color.WHITE
                canvas.drawText("SERVER OFFLINE", x, y, paint)
                y += 32f
                paint.textSize = 13f
        ...
        private fun percent(v: Double) =
            String.format(Locale.US, "%5.1f%%", v)
